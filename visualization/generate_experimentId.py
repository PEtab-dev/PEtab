...
def generate_experimentId(measurement_data):
    ...
    observableTransformation = np.array(
        measurement_data.observableTransformation
    )

    # check if there occurs the name 'nan' in the observableParameter column
    # , should not occur!
    ...
    tmp_observableParameters[tmp_ind[0][
        np.where(isnanVectorized(observableParameters[tmp_ind[0]]))]
    ] = 'empty'
    ...
    while ind_noExpId[0].size > 0:
        ind_expId = np.where(
            (observableParameters == observableParameters[ind_noExpId[0][0]]) *
            (tmp_noiseParameters == tmp_noiseParameters[ind_noExpId[0][0]]) *
            (observableTransformation ==
             observableTransformation[ind_noExpId[0][0]]))
